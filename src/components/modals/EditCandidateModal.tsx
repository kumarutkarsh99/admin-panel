...
export const EditCandidateModal: React.FC<EditCandidateModalProps> = ({
  ...
  const ratingStyles: Record<number, { selectedBg: string; hoverBg: string }> =
    {
      ...
      5: { selectedBg: "bg-blue-600 text-white", hoverBg: "hover:bg-blue-200" },
    };
  ...
  const addTag = () => {
    ...
    if (tag && !formData.skill.includes(tag)) {
      setFormData((prev) => ({ ...prev, skill: [...prev.skill, tag] }));
    }
    setTagInput("");
  };
  ...
  const validateForm = (): CandidateFormKey | null => {
    const newErrors: Partial<Record<CandidateFormKey, string>> = {};
    if (!formData.first_name) newErrors.first_name = "First Name is required";
    if (!formData.email) newErrors.email = "Email is required";
    if (!formData.phone) newErrors.phone = "Phone number is required";
    if (!formData.resume_url) newErrors.resume_url = "Resume link is required";
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/candidate/${candidateId}`, payload);
      toast.success("Candidate updated successfully");
      onOpenChange(false);
    } catch (err: unknown) {
      if (axios.isAxiosError(err)) {
        toast.error(err.response?.data?.message || "Error updating candidate");
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl overflow-y-auto max-h-[80vh]">
        <DialogHeader>
